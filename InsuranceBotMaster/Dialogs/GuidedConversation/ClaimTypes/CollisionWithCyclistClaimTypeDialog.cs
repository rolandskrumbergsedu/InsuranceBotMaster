...
namespace InsuranceBotMaster.Dialogs.GuidedConversation.ClaimTypes
{
    ...
    public class CollisionWithCyclistClaimTypeDialog : IDialog<object>
    {
        ...
        {
            context.Call(new CyclistInjuredDialog(), CyclistInjuredDialogResumeAfter);
        }

        private async Task CyclistInjuredDialogResumeAfter(IDialogContext context, IAwaitable<object> result)
        {
        ...
        {
            context.Call(new CycleVehicleDamageDialog(), CycleVehicleDamageDialogResumeAfter);
        }

        private async Task CycleVehicleDamageDialogResumeAfter(IDialogContext context, IAwaitable<object> result)
        {
